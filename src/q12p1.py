...
def main():
    ...
    print(f"{'':>12}" + "".join(f"{point!s:^12}" for point in generated_points))
    for point_1 in generated_points:
        print(f"{point_1!s:^12}" + "".join(f"{curve.add_affine_points(point_1, point_2)!s:^12}" for point_2 in generated_points))
